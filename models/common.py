...
class CAL(nn.Module):
    def __init__(self, c1, c2, ratio=16, kernel_size=7):
        ...
        self.cv1 = CoT(c1)
        self.cv2 = PSAModule(c1, c2)
        self.spatial_attention = SpatialAttention(kernel_size)

    def forward(self, x):
        k1 = self.cv1(x)
        k2 = self.spatial_attention(k1)
        # out = self.spatial_attention(k1)
        return k2
...
class SAtt(nn.Module):
    def __init__(self, c1,):
        ...
        self.cv2 = nn.Conv2d(c1//16,c1//16,kernel_size=3,dilation=1,padding=1)
        self.cv3 = nn.Conv2d(c1//16,c1//16,kernel_size=3,dilation=3,padding=3)
        self.cv4 = nn.Conv2d(c1//16,c1//16,kernel_size=3,dilation=5,padding=5)
...
class SAtt2(nn.Module):
    def __init__(self,c1,):
        super(SAtt2, self).__init__()
        self.cv1 = nn.Conv2d(c1,c1//16,kernel_size=1,)
